...
const syncIcon = async () => {
  ...
  // Defina os atributos
  svg.setAttribute("class", "status-sync spin360");
...
const modalHide = async () => {
  ...
  modal.querySelector('section').replaceChildren('');
...
const modalMsg = async (text, title = false, closeClick = false, icon = false) => {
  ...
  if (title !== false) modal.querySelector('section > label').textContent = title;

  if (icon !== false && !modal.querySelector('section > svg')) {
    modal.querySelector('section').appendChild(await syncIcon());
  }
  
  modal.querySelector('section > p').textContent = text;
...
const modalShow = async (text, title, closeClick = false, icon = false) => {
  const overlay = document.getElementById('modal-overlay');
  const modal = document.getElementById('modal-progress');
  ...
  modal.querySelector('section').appendChild(label);
  
  if (icon !== false && !modal.querySelector('section > svg')) {
    modal.querySelector('section').appendChild(await syncIcon());
  }

  modal.querySelector('section').appendChild(paragraph);
  
  if (closeClick) {
    const btnOK = document.createElement('button');
    btnOK.textContent = 'OK';
    btnOK.onclick = async () => await modalHide();
    modal.querySelector('section').appendChild(btnOK);
  }
...
document.getElementById('sync').addEventListener('click', async () => {
  ...
  const result = await window.data.sync();
  
  if (result) await modalMsg(
